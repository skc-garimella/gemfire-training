...
public class RemoteLauncherCommands implements CommandMarker {
         ...
         final String[] jvmArgsOpts)
 {
  ...
  public Result startServer(@CliOption(key = "host",
          ...
          final String group,
          @CliOption(key =  "license-application-cache",
                    unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
                    help = "Specifies the serial number this distributed system member will use unless a <code>license-data-management</code> serial number is also specified. This distributed member will attempt to activate a GemFire Application Cache Node license which allows it to be a peer-to-peer application cache node. If a <code>license-data-management</code> serial number is provided, then the <code>license-application-cache</code> serial number may be specified as the license for cache clients connecting to this node.")
          final String licenseApplicationCache,
          @CliOption(key = "license-data-management",
                    unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
                    help = "Specifies the serial number(s) this distributed system member will use. This distributed member will attempt to activate a GemFire Data Management Node license which allows it to provide advanced data management services which includes hosting a cache server to which cache clients may connect.")
          final String licenseDataManagement,
          @CliOption(key = "locators",
                    unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
